...
- Bump `package.json` version on every PR so package update flows pick it up.
...
- Run `pi update --extensions` locally after publish-worthy changes when verifying install behavior.
- **Push to `origin` after every release.** Package update flows pull from this repo's `origin` remote. Do not push to additional remotes unless explicitly asked.
