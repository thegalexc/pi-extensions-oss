...
- clear working prompt and calm tool status on Pi `agent_settled` so queued follow-ups, retries, and compaction retries keep status visible until the run is truly idle
...
- bump Pi development dependencies to `^0.78.0` so local validation tracks the current Pi 0.78 line
...
- refresh the lockfile to the current Pi 0.75.4 dependency graph, including the upstream install hardening and update-note changes in that patch window

## 0.2.37

- bump Pi development dependencies to `^0.75.3` so local validation tracks the audited Pi 0.75.3 line
- refresh the lockfile to the current Pi 0.75.3 dependency graph, including the upstream CLI fixes in that patch window
